...
public class MinionsItemCompactRepositoryImpl implements MinionsItemCompactRepository {
    ...
    private RowMapper<ItemAction> compactRowMapper = row -> ItemAction.builder()
    ...
    public Optional<ItemAction> getItemCompactByItemName(String itemName) {
        return simpleJDBSTemplate.query(SQL_SELECT_BY_ITEM_NAME, compactRowMapper, itemName).stream().findAny();
    }

    @Override
    public void save(ItemAction entity) {
        simpleJDBSTemplate.query(SQL_INSERT_ENTRY, compactRowMapper, entity.getItemName(), entity.getResultName(),
                entity.getInAmount(), entity.getOutAmount());
    }

    @Override
    public void update(ItemAction entity) {
        simpleJDBSTemplate.query(SQL_UPDATE, compactRowMapper, entity.getResultName(), entity.getInAmount(),
                entity.getOutAmount(), entity.getItemName());
    }

    @Override
    public void delete(ItemAction entity) {
        simpleJDBSTemplate.query(SQL_DELETE, compactRowMapper, entity.getItemName());
    }
    ...
    @Override
    public List findAll() {
        return simpleJDBSTemplate.query(SQL_SELECT_ALL, compactRowMapper);
    }
}
